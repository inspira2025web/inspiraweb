...
<?php include 'assets/includes/header.php'; ?>
     ...
     <main id="page-content" class="min-h-screen w-full flex flex-col items-center justify-center mt-5 p-4 sm:p-6 lg:p-8">
        
        <div id="form-container" class="w-full max-w-lg mx-auto mt-4">
             ...
             <div id="ticket" class="ticket-gradient text-white rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div class="bg-black/20 px-6 md:px-8 py-4 flex items-center justify-between">
                    ...
                    <div class="bg-white p-1 rounded-md">
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=www.inspira.x10.mx" alt="QR Code for INSPIRA 2025" class="w-20 h-20">
                    </div>
                </div>
            </div>
...
const formActionUrl = 'https://script.google.com/macros/s/AKfycbzlSItl9zSmihUR6AJ_AruUtaHBz_WT2y-EOdVJlXVg-13A-X6Z16HdLNHPZW0VLgOKcA/exec';
...
function validateRegNumber(regNumber) {
    ...
    if (prefix in ranges) {
        // Auto-pad number to 3 digits
        const paddedNumStr = num.toString().padStart(3, '0');

        // Check if padded string matches original (or original is shorter)
        if (!/^\d{3}$/.test(paddedNumStr)) return false;

        // Check range
        if (num >= 0 && num <= ranges[prefix]) {
            return true;
        }
    }
...
// Example: pad input automatically
ticketForm.addEventListener('submit', function (e) {
    ...
    // Auto-pad number part
    const parts = regNumber.split('/');
    if (parts.length === 3) {
        let num = parseInt(parts[2], 10);
        if (!isNaN(num)) {
            parts[2] = num.toString().padStart(3, '0');
            regNumber = parts.join('/');
            regNumberInput.value = regNumber; // update input field
        }
    }
